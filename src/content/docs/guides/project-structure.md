...
│   ├── App.tsx           ← 主页面：聊天流、状态管理、流式渲染
│   ├── App.css           ← 全局样式（暗色主题 + GitHub Dark 风格）
│   ├── types/product.ts  ← 数据类型定义（商品、消息、设置）
│   ├── api/
...
│   └── components/
│       ├── chat/
│       │   ├── ChatInput.tsx       ← 底部输入栏，支持 Shift+Enter
│       │   └── ThinkingBlock.tsx   ← LLM 思考过程可展开
│       ├── results/
│       │   ├── ResultCard.tsx      ← 商品详情卡片 + Drawer
│       │   └── PriceComparison.tsx ← 价格对比图 + 导出
│       ├── settings/
│       │   └── SettingsModal.tsx   ← 模型设置弹窗
│       └── sidebar/
│           ├── Sidebar.tsx         ← 侧栏容器
...
- **Sidebar + ConversationList**：对话历史侧栏，多轮追问上下文记忆
